...
def opening_theorem(statement: str) -> KeepTogether:
    return KeepTogether(
        [
            Rule(color=INK, thickness=0.75, space_before=5, space_after=7),
            Paragraph("Theorem (six-state counterexample).", THEOREM_LABEL),
            Paragraph(statement, THEOREM),
            Rule(color=INK, thickness=0.75, space_before=0, space_after=8),
        ]
    )
...
def build_story(width: float):
    story = [
        ...
        opening_theorem(
            "The answer to Open Problem 3.11 of Grimmett and Holmes is negative: on six states, let A=(0,1,0,0,1,0) and C=(3,4,5,1,2,1), and choose each successive map independently to be A or C with probability 1/2 each; the induced Markov chain is irreducible and aperiodic, and the grand coupling has exactly two terminal coalescence classes almost surely, with size profile (2,4) with probability 2/3 and (3,3) with probability 1/3."
        ),
